	 ...
	 */
	 function goStartMode(){
		 ...
		 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
			 welBtnConfirm.attr("src","../img/photo/btn_confirm2.png");
	 ...
	 function goReadyMode(){
		 ...
		 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
			 ...
			 welBtnConfirm.attr("src","../img/photo/btn_confirm.png");
		 }
	 }   
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					alert("JPG, GIF, PNG, BMP 형식의 이미지만 업로드 가능합니다.");
	...
	function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			...
			aFileList.push('	<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button" src="../img/photo/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
			aFileList.push('	</li> ');   
	...
	function html5Upload() {	
		...
		sUploadURL= 'fileUploaderHtml5.aspx'; 	//upload URL
	...
	function callAjaxForHTML5 (tempFile, sUploadURL){
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						alert("JPG, GIF, PNG, BMP 형식의 이미지만 업로드 가능합니다. ("+sFileName+")");
					}else{
		...
		oAjax.header("fileName",encodeURIComponent(tempFile.name));
		oAjax.header("fileSize",tempFile.size);
		oAjax.header("fileType",tempFile.type);
	...
	function onAjaxError (){
		alert("Error 0 : 해당 오류가 반복해서 일어난다면 서버 측에 문의하십시오");
	}
	...
	function callFileUploader (){
		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/fileUploader.aspx',			//샘플 URL입니다.
			sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',			//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
			sFiletype : "*.jpg;*.jpeg;*.png;*.bmp;*.gif",									//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)	
			sMsgNotAllowedExt : 'JPG, GIF, PNG, BMP 형식의 이미지만 업로드 가능합니다.',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
